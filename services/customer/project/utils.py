...
def split_url(url, method='GET'):
    ...
    if appctx is None:
        raise RuntimeError('Intento hacer coincidir una URL sin el'
                           'contexto de aplicacion siendo empujado. Esto tiene que ser'
                           'ejecutado cuando el contexto de la aplicacion esta disponible.')
    ...
    if reqctx is not None:
        ...
        if url_adapter is None:
            raise RuntimeError('La aplicacion no pudo crear una URL'
